    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        ...
        when (event.action) {
            KeyEvent.ACTION_DOWN -> {
                // Hoenn Forge: Start button opens save-state menu (not in-game Start)
                if (event.keyCode == KeyEvent.KEYCODE_BUTTON_START && event.repeatCount == 0) {
                    emulationFragment.openHoennQuickMenu()
                    return true
                }
            ...
            KeyEvent.ACTION_UP -> {
                // Consume Start up so it never reaches the emulated 3DS Start button
