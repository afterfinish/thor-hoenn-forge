...
class HoennPrefs(context: Context) {
    ...
    /** Outdoor free look: multiplier on the yaw/pitch clamps. */
    var gpuCamRange: Float
    ...
    var gpuCamPivotY: Float
        get() = prefs.getFloat(KEY_GPUCAM_PIVOT_Y, 60f)
    ...
    companion object {
        ...
        private const val GPUCAM_GEN = 8
    ...
    init {
        ...
        if (prefs.getInt(KEY_GPUCAM_GEN, 0) < GPUCAM_GEN) {
            prefs.edit {
                ...
                remove(KEY_GPUCAM_PIVOT_Y)
                remove(KEY_GPUCAM_RANGE)
